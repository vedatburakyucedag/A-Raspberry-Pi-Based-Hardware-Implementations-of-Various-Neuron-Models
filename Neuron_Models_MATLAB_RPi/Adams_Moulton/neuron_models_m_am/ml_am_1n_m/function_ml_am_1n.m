function [V,W] = function_ml_am_1n(E_Ca,E_K,E_Leak,g_Ca,g_K,g_Leak,V1,V2,V3,V4,T0,I,hh,N,t,V,W,C)
...
fW=@(t,V,W)  ((0.5 * (1 + tanh((V - V3)/V4)) - W) / ((T0) / (cosh((V - V3) / (2 * V4)))));
    ...
    for i=3:N
        ...
        V(i+1) = V(i) + hh/24 * (9*fV(t(i+1),V(i+1),W(i+1)) + 19*fV(t(i),V(i),W(i)) - 5*fV(t(i-1),V(i-1),W(i-1)) + fV(t(i-2),V(i-2),W(i-2)));
        W(i+1) = W(i) + hh/24 * (9*fW(t(i+1),V(i+1),W(i+1)) + 19*fW(t(i),V(i),W(i)) - 5*fW(t(i-1),V(i-1),W(i-1)) + fW(t(i-2),V(i-2),W(i-2)));
