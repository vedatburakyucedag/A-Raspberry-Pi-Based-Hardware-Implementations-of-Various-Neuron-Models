...
a=1; b=3; c=1; d=5; s=4; e=0.006; x0=-1.56; x=-1.0908; y=-4.8828; z=3.0912; I=3.1; # chaotic
...
def fx5(x, y, z, dx1,dx2,dx3,dx4, dy1,dy2,dy3,dy4,dz1,dz2,dz3,dz4):
    fxx5 = x + hh * ((5 / 32) * dx1 + (7 / 32) * dx2 + (13 / 32) * dx3 - (1 / 32) * dx4)
    fxy5 = y + hh * ((5 / 32) * dy1 + (7 / 32) * dy2 + (13 / 32) * dy3 - (1 / 32) * dy4)
    fxz5 = z + hh * ((5 / 32) * dz1 + (7 / 32) * dz2 + (13 / 32) * dz3 - (1 / 32) * dz4)
    return fxx5, fxy5, fxz5
...
while 1:
    ...
    fxx5, fxy5, fxz5 = fx5(x, y, z, dx1, dx2, dx3, dx4, dy1, dy2, dy3, dy4, dz1, dz2, dz3, dz4)
    dx5, dy5, dz5 = df(fxx5, fxy5, fxz5)

    x = x + (hh / 6) * (dx1 + 2 * dx2 + 2 * dx3 + dx5)
    y = y + (hh / 6) * (dy1 + 2 * dy2 + 2 * dy3 + dy5)
    z = z + (hh / 6) * (dz1 + 2 * dz2 + 2 * dz3 + dz5)
