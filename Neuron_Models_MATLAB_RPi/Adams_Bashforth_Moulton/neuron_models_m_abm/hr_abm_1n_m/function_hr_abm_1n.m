function [xx,yy,zz] = function_hr_abm_1n(x,y,z,t,a,b,c,d,s,e,x0,h,N,I)
...
fz=@(t,x,z)     e * ( s * ( x - x0 ) - z );
    ...
    for i=4:N
        ...
        xx(i+1) = x(i) + h/24 * (9*fx(t(i+1),x(i+1),y(i+1),z(i+1)) + 19*fx(t(i),x(i),y(i),z(i)) - 5*fx(t(i-1),x(i-1),y(i-1),z(i-1)) + fx(t(i-2),x(i-2),y(i-2),z(i-2)));
        yy(i+1) = y(i) + h/24 * (9*fy(t(i+1),x(i+1),y(i+1)) + 19*fy(t(i),x(i),y(i)) - 5*fy(t(i-1),x(i-1),y(i-1)) + fy(t(i-2),x(i-2),y(i-2)));
        zz(i+1) = z(i) + h/24 * (9*fz(t(i+1),x(i+1),z(i+1)) + 19*fz(t(i),x(i),z(i)) - 5*fz(t(i-1),x(i-1),z(i-1)) + fz(t(i-2),x(i-2),z(i-2)));
