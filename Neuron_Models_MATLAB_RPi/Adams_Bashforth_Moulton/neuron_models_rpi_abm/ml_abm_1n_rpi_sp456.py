...
#C = 1; E_Ca = 50; E_K = - 100; E_Leak = - 70; g_Ca = 20; g_K = 20; g_Leak = 2; V1 = -12; V2 = 18; V3 = -10; V4 = 13; T0 = 7; I = 100; V = -12.7743548965288; W = 0.376576642801617;

#C=1; E_Ca=120; E_K=-80; E_Leak=-60; g_Ca=4;   g_K=8; g_Leak=2; V1=-1.2; V2=18; V3=14.95; V4=17.4; T0=15; I=54; V = -32.6940; W = 0.0294; # ch1
C=1; E_Ca=120; E_K=-80; E_Leak=-60; g_Ca=4;   g_K=8; g_Leak=2; V1=-1.2; V2=18; V3=12;    V4=17.4; T0=15; I=40; V = -32.6940; W = 0.0294; # ch2
#C=5; E_Ca=120; E_K=-84; E_Leak=-60; g_Ca=4.4; g_K=8; g_Leak=2; V1=-1.2; V2=18; V3=2;     V4=30;   T0=5;  I=120;V = -46.9606; W = 0.3221; # ch3
...
while 1:
    ...
    fV3 = (I - ((g_Ca * 0.5 * (1 + math.tanh((v1[3] - V1) / V2)) * (v1[3] - E_Ca)) + (g_K * w1[3] * (v1[3] - E_K)) + (
                g_Leak * (v1[3] - E_Leak)))) / C
    fW3 = (0.5 * (1 + math.tanh((v1[3] - V3) / V4)) - w1[3]) / ((T0) / (math.cosh((v1[3] - V3) / (2 * V4))))

    fV2 = (I - ((g_Ca * 0.5 * (1 + math.tanh((v1[2] - V1) / V2)) * (v1[2] - E_Ca)) + (g_K * w1[2] * (v1[2] - E_K)) + (
                g_Leak * (v1[2] - E_Leak)))) / C
    fW2 = (0.5 * (1 + math.tanh((v1[2] - V3) / V4)) - w1[2]) / ((T0) / (math.cosh((v1[2] - V3) / (2 * V4))))

    fV1 = (I - ((g_Ca * 0.5 * (1 + math.tanh((v1[1] - V1) / V2)) * (v1[1] - E_Ca)) + (g_K * w1[1] * (v1[1] - E_K)) + (
                g_Leak * (v1[1] - E_Leak)))) / C
    fW1 = (0.5 * (1 + math.tanh((v1[1] - V3) / V4)) - w1[1]) / ((T0) / (math.cosh((v1[1] - V3) / (2 * V4))))

    fV0 = (I - ((g_Ca * 0.5 * (1 + math.tanh((v1[0] - V1) / V2)) * (v1[0] - E_Ca)) + (g_K * w1[0] * (v1[0] - E_K)) + (
                g_Leak * (v1[0] - E_Leak)))) / C
    fW0 = (0.5 * (1 + math.tanh((v1[0] - V3) / V4)) - w1[0]) / ((T0) / (math.cosh((v1[0] - V3) / (2 * V4))))
