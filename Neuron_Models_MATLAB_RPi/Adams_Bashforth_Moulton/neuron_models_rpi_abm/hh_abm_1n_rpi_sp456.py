...
for i in range(0, 3):
        ...
        V = (V + (hh / 6) * (dV1 + 2 * dV2 + 2 * dV3 + dV4)); V1.append(V)
        h = (h + (hh / 6) * (dh1 + 2 * dh2 + 2 * dh3 + dh4)); h1.append(h)
        m = (m + (hh / 6) * (dm1 + 2 * dm2 + 2 * dm3 + dm4)); m1.append(m)
        n = (n + (hh / 6) * (dn1 + 2 * dn2 + 2 * dn3 + dn4)); n1.append(n)
...
while 1:
    ...
    fV3, fh3, fm3, fn3 = df(I_ion,alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n, h1[3], m1[3], n1[3], a, f, t)

    alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n = alpha_beta(Vrest,V1[2])
    I_ion = current(V1[2], h1[2], m1[2], n1[2])
    fV2, fh2, fm2, fn2 = df(I_ion,alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n, h1[2], m1[2], n1[2], a, f, t)

    alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n = alpha_beta(Vrest,V1[1])
    I_ion = current(V1[1], h1[1], m1[1], n1[1])
    fV1, fh1, fm1, fn1 = df(I_ion,alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n, h1[1], m1[1], n1[1], a, f, t)

    alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n = alpha_beta(Vrest,V1[0])
    I_ion = current(V1[0], h1[0], m1[0], n1[0])
    fV0, fh0, fm0, fn0 = df(I_ion,alpha_h, alpha_m, alpha_n, beta_h, beta_m, beta_n, h1[0], m1[0], n1[0], a, f, t)

    V = V1[3] + hh / 24 * (55 * fV3 - 59 * fV2 + 37 * fV1 - 9 * fV0)
    h = h1[3] + hh / 24 * (55 * fh3 - 59 * fh2 + 37 * fh1 - 9 * fh0)
    m = m1[3] + hh / 24 * (55 * fm3 - 59 * fm2 + 37 * fm1 - 9 * fm0)
    n = n1[3] + hh / 24 * (55 * fn3 - 59 * fn2 + 37 * fn1 - 9 * fn0)
